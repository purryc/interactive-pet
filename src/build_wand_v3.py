"""Build the articulated Heihei cat wand; Blender 5.2, deterministic."""
import math
import os
import sys
...
GLB = ROOT / "web/public/assets/cat_wand_v3.glb"
BLEND = ROOT / "output/cat_wand_v3.blend"
PREVIEW = ROOT / "qa/web/wand_v3_preview.png"
...
def rig_plume(name, meshes, length):
    """Three independently weighted joints preserve a continuous feather surface."""
    data=bpy.data.armatures.new(name+"_bones")
    ...
    for index in range(3):
        bone=data.edit_bones.new(f"{name}_joint_{index}")
        bone.head=(0,0,-.026-length*index/3)
        bone.tail=(0,0,-.026-length*(index+1)/3)
        if previous:
    ...
    for mesh in meshes:
        if mesh.type!="MESH":
            continue
        mesh.parent=rig
        groups=[mesh.vertex_groups.new(name=f"{name}_joint_{index}") for index in range(3)]
        for vertex in mesh.data.vertices:
            progress=max(0,min(2,(abs(vertex.co.z+.026)/length)*3-.5))
            lower=min(2,int(progress))
            upper=min(2,lower+1)
            blend=progress-lower
            groups[lower].add([vertex.index],max(.001,1-blend),"REPLACE")
            if upper!=lower and blend>.001:
                groups[upper].add([vertex.index],blend,"REPLACE")
        deform=mesh.modifiers.new(name+"_skin","ARMATURE")
        deform.object=rig
    return rig


def articulated_feather(name, side, length, width, mat, lean, phase, yaw):
    before=set(bpy.context.scene.objects)
    feather(name,side,length,width,mat,lean,phase,yaw)
    fibers=[]
    for k in range(10):
        radial=(k/10-.5)*width*.55
        points=[]
        for j in range(5):
            t=j/4
            x=side*.010+radial+lean*t*t*.55
            y=phase*.008+.002*math.sin(k*3.1)
            z=-.032-length*(.25+.39*k/10)*t
            points.append((x*math.cos(yaw)-y*math.sin(yaw),x*math.sin(yaw)+y*math.cos(yaw),z))
        fibers.append(tube(name+"_fiber",points,.00028,cream if k%3 else gold,lure,1))
    bpy.ops.object.select_all(action="DESELECT")
    for obj in fibers:obj.select_set(True)
    bpy.context.view_layer.objects.active=fibers[0]
    bpy.ops.object.join()
    fibers[0].name=name+"_fibers"
    new=[obj for obj in bpy.context.scene.objects if obj not in before]
    rig_plume(name,new,length)


articulated_feather("Honey_plume", 0, .177, .033, gold, .031, 0, .15)
articulated_feather("Pale_plume", -1, .146, .025, cream, -.033, 1, .72)
articulated_feather("Ochre_plume", 1, .164, .026, ochre, .041, -1, -.48)
articulated_feather("Narrow_plume", 0, .127, .017, cream, -.019, -2, 1.24)
articulated_feather("Soft_side_plume", 1, .142, .020, gold, .012, 2, 2.08)
...
for obj in bpy.context.scene.objects:
    obj.select_set(obj == rod or obj == lure or obj.parent in {rod,lure} or obj.parent is not None and obj.parent.parent==lure)
...
camera.rotation_euler=direction.to_track_quat("-Z","Y").to_euler()
bpy.context.scene.render.filepath=str(ROOT / "qa/web/wand_v3_detail.png")
bpy.ops.render.render(write_still=True)
bpy.context.scene.render.filepath=str(PREVIEW)
