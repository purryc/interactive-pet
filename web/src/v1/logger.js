const columns=['timestamp','source','poseSource','poseVaried','rawX','rawY','rawHeight','filteredX','filteredY','filteredHeight','azimuth','altitude','velocityX','velocityY','velocityZ','featherX','featherY','featherZ','catX','catY','catZ','catState','catAction','targetDistance','targetHeight','gestureState','contactPart','contactStrength'];
...
export class InteractionLogger {
  ...
  record(dt,{raw,filtered,feather,cat,brain,perception,gesture}){
    ...
    this.rows.push({timestamp:new Date().toISOString(),source:raw?.source??raw?.type,poseSource:raw?.orientation?.source,poseVaried:raw?.orientation?.observedVariation,rawX:raw?.position.x,rawY:raw?.position.z,rawHeight:raw?.position.y,filteredX:filtered?.position.x,filteredY:filtered?.position.z,filteredHeight:filtered?.position.y,azimuth:raw?.orientation.azimuth,altitude:raw?.orientation.altitude,velocityX:raw?.velocity.x,velocityY:raw?.velocity.z,velocityZ:raw?.velocity.y,featherX:feather?.position.x,featherY:feather?.position.z,featherZ:feather?.position.y,catX:cat.root.position.x,catY:cat.root.position.z,catZ:cat.root.position.y,catState:brain.state,catAction:cat.animation.current,targetDistance:perception.targetDistance,targetHeight:perception.targetHeight,gestureState:gesture?.state,contactPart:brain.lastContact?.part,contactStrength:brain.lastContact?.strength});
  }
