...
const pointerCopy=p=>({...p,position:p.position.clone(),velocity:p.velocity.clone(),screen:{...p.screen},interaction:{...p.interaction},orientation:{...p.orientation}});
export class SpatialInputSystem {
  constructor(canvas,camera){
    ...
    this.onChange=null;this.mode='wand';this.lastPenEvent=null;
    this.move=e=>this.handlePointer(e);this.leave=e=>{if(e.pointerType!=='touch')this.deactivate();};
  ...
  setSimulatedHeight(value){this.heightMapper.simulated=value;if(this.rawPointer&&this.rawPointer.heightSource==='模拟高度'){this.rawPointer.position.y=value;this.filteredPointer.position.y=value;}}
  handlePointer(e){
    ...
    const position=mapped.world.clone();position.y=height;
    const velocity=this.lastPosition?position.clone().sub(this.lastPosition).divideScalar(dt):new Vector3();
    const altitude=Number.isFinite(e.altitudeAngle)?e.altitudeAngle:Math.PI/2;
    const azimuth=Number.isFinite(e.azimuthAngle)?e.azimuthAngle:0;
    const previousSpeed=this.rawPointer?.speed??0;
    const raw={position,velocity,speed:velocity.length(),active:true,timestamp,screen:mapped.screen,interaction:mapped.interaction,orientation:{azimuth,altitude},type:e.pointerType||'mouse',contact:e.buttons!==0,heightSource:physical===null?'模拟高度':'设备距离',physicalHeight:physical};
    ...
    if(!this.filteredPointer)this.filteredPointer=pointerCopy(raw);
      ...
      filtered.orientation.azimuth=MathUtils.damp(filtered.orientation.azimuth,azimuth,this.tiltSmoothing,dt);
      filtered.orientation.altitude=MathUtils.damp(filtered.orientation.altitude,altitude,this.tiltSmoothing,dt);
  ...
  deactivate(){if(this.rawPointer)this.rawPointer.active=false;if(this.filteredPointer)this.filteredPointer.active=false;this.lastTime=null;this.lastPosition=null;this._previousFilteredPosition=null;this.onChange?.(this.rawPointer,this.filteredPointer);}
  dispose(){this.canvas.removeEventListener('pointermove',this.move);this.canvas.removeEventListener('pointerdown',this.move);this.canvas.removeEventListener('pointerleave',this.leave);}
}
